...
def validate_record(
    ...
    # 2. Formato inválido
    pages, parse_err = parse_folio_range(str(folio_val))
    if parse_err:
    ...
    if min_page < 1:
        return False, f"Folio '{folio_val}' resulta en número de página < 1"

    # 4. Detección de salto de secuencia
    if check_gaps and prev_last_page is not None:
        current_first = first_page_of_range(str(folio_val))
